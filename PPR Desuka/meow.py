...
parser = argparse.ArgumentParser(description="PopPopRet wa doko desuka?")
parser.add_argument('badchars', nargs=1, help="list of badchars separated by new lines")
parser.add_argument('findjump2output', nargs=1, help="output file of findjump2.exe")
args = parser.parse_args()

allchars = ("\\00\\x01\\x02\\x03\\x04\\x05\\x06\\x07\\x08\\x09\\x0a\\x0b\\x0c\\x0d\\x0e\\x0f\\x10\\x11\\x12\\x13"
...
def filterthebadguys(filetobadchars):
    whereallthegoodcharsgather = allchars
    with open(filetobadchars,"r") as filebadchars:
...
def main():
    goodguys = filterthebadguys(args.badchars[0])
    with open(args.findjump2output[0],"r") as inputfile:
        line = inputfile.readlines()
        for each in line:
            if each[0] == "0" and each[1] == "x":
                if each.rstrip().split()[1] == "pop":
                    addr = each.rstrip()[2:10].lower()
                    if addr[0:2] in goodguys :
                        if addr[2:4] in goodguys:
                            if addr[4:6] in goodguys:
                                if addr[6:8] in goodguys:
                                    print(each.rstrip())
